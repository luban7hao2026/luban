...
- `400`: 该食物名称已存在，请换一个名称（同一用户下名称不区分大小写不可重复）。
- `403`: 用户被禁用。
...
- `400`: 该食物名称已存在，请换一个名称（默认模板用户下名称不区分大小写不可重复）。

### PATCH /admin/default-foods/{food_id}

更新默认模板食物。修改后会自动同步到所有真实用户名下同名的食物，覆盖其名称、分类、图片（不修改各用户的启用状态）；若某用户已存在与新名称相同的食物，则跳过该用户以避免名称冲突。
...
删除默认模板食物。同时删除所有真实用户名下同名的食物及其相关抽取记录。
...
- `400`: 该食物名称已存在，请换一个名称（该用户下名称不区分大小写不可重复）。
...
- 管理员修改默认食物时，会按名称同步更新所有真实用户名下同名食物的名称、分类、图片；管理员删除默认食物时，会删除所有真实用户名下同名食物及其抽取记录。
- 同一用户下食物名称不区分大小写不可重复，由数据库唯一索引 `uq_foods_user_name (user_id, name)` 兜底，任何新增/修改/批量导入的重复都会被拒绝。
- 用户之间的食物和抽取记录互相隔离。
